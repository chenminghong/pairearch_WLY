//
//  OrderStatusBACK220Controller.m
//  pairearch_WLY
//
...
@interface OrderStatusBACK220Controller ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation OrderStatusBACK220Controller
...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        [self.footerView.startTransportBtn setTitle:@"签到确认" forState:UIControlStateNormal];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *titleText = [OrderStatusManager getOrderDescriptionWithStatus:ORDER_STATUS_220 orderType:ORDER_TYPE_BACK];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        cell.descriptionLabel.text = [OrderStatusManager getOrderDescriptionWithStatus:ORDER_STATUS_220 orderType:ORDER_TYPE_BACK];
    } else {
...
- (void)startTransportAction:(UIButton *)sender {
    ...
    NSDictionary *paraDict = @{@"driverTel":driverTel, @"orderCode":orderCode, @"orderCodes":orderCodes, @"type":@"FRM"};
